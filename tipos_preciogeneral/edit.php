...
$sql=mysqli_query($enlaceCon,"select nombre, abreviatura,desde,hasta,glosa_factura,glosa_estado,monto_inicio,monto_final from $table where codigo=$codigo_registro");
...
$monto_ini=$dat['monto_inicio'];
$monto_fin=$dat['monto_final'];
$activado="";
$desactivado="";
if($glosa_estado==1){
	$activado="selected";
...
echo "<form action='$urlSaveEdit' id='precio_general' method='post'>";
...
echo "<td align='left'>
	<INPUT  type='number' class='form-control' value='$monto_ini' id='monto_ini' name='monto_ini' required>
</td><td align='left' class='bg-info text-white'>Hasta $</td><td align='left' colspan='3'>
	<INPUT  type='number' class='form-control' value='$monto_fin' id='monto_fin' name='monto_fin' required>
</td>";
echo "</tr>";
