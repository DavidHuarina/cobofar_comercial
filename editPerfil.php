...
if(isset($_POST["clave_ant"])){
 ...
 if($clave_ant==""||$clave==""){
    ...
    $sql = "SELECT u.codigo_funcionario FROM usuarios_sistema u
      WHERE u.codigo_funcionario='$user' AND u.contrasena= BINARY '$clave_ant' ";
    $resp = mysqli_query($enlaceCon,$sql);
    $num_filas = mysqli_num_rows($resp);
    if ($num_filas != 0) {
     if($clave==$clave2){
     	if(strlen($clave)>=6){
           if(preg_match_all( "/[A-Z]/", $clave)>0){
               if(preg_match_all( "/[0-9]/", $clave)>0){
                   $sqlContra = "UPDATE usuarios_sistema SET contrasena= BINARY '$clave' where codigo_funcionario='$user'";
...
<aside class="profile-card">
  ...
  <div class="input-group mb-2 mr-sm-2">
    <div class="input-group-prepend">
      <div class="input-group-text"><i class="material-icons text-primary">vpn_key</i></div>
    </div>
