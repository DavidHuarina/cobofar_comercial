...
$sql="select f.nro_factura, DATE_FORMAT(f.fecha, '%d/%m/%Y'), f.importe, f.razon_social, f.nit, d.nro_autorizacion, e.abreviatura, f.codigo_control,(SELECT descripcion FROM ciudades where cod_ciudad=f.cod_sucursal)nombre_ciudad,s.cod_tipo_doc,DATE_FORMAT(s.fecha_anulacion, '%d/%m/%Y')fecha_anulacion
	...
	$sqlTipo and s.estado_salida=3 order by f.fecha, f.nro_factura";
//echo $sql;	
