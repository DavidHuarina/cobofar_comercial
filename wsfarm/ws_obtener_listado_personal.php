...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(isset($datos['accion'])&&isset($datos['sIdentificador'])&&isset($datos['sKey']))
        ...
        $estado=0;
        $mensaje="";
        if($accion=="ObtenerListadoPersonal"){
          ...
          $datosResp=obtenerDatosPersonalDatos($sta);                
          if($datosResp[0]==0){
                 $estado=2;
                 $mensaje = "Lista Vacia";
...
function obtenerDatosPersonalDatos($estado){
  ...
  $sql="SELECT p.* FROM ppersonal p where p.idper>0 $sqlSta";
  $stmt = $dbh->prepare($sql);
  $stmt->execute();
  $ff=0;
  $datos=[];
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
     $datos[$ff]['idper']=$row['IDPER'];
     $datos[$ff]['cargo']=$row['CARGO'];
     $datos[$ff]['paterno']=$row['PATERNO'];
     $datos[$ff]['materno']=$row['MATERNO'];
     $datos[$ff]['nombre']=$row['NOMBRE'];
     $datos[$ff]['fechnac']=$row['FECHNAC'];
     $datos[$ff]['dir']=$row['ZONA']." ".$row['DIR'];
