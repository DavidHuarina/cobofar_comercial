...
echo "<table align='center' class='textotit'><tr><th>Proceso de Costeo de Materiales PEPS</th></tr></table>";
...
while($datMateriales=mysqli_fetch_array($respMateriales)){
	...
	while($datCostos=mysqli_fetch_array($respCostos)){
		...
		$respDelCosto=mysqli_query($enlaceCon,$sqlDelCosto);
		...
		$respInsertCosto=mysqli_query($enlaceCon,$sqlInsertCosto);
	...
	{	$cod_salida_almacen=$dat_salida_detalle[0];
		...
		$sql_detalle_ingreso="select c.`cod_ingreso`, c.`cantidad_restante`, c.costo_unitario from `costos_mes_pueps` c 
			...
			c.cantidad_restante>0 order by c.`fecha_ingreso`";
