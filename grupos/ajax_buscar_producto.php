...
$sql="(SELECT s.cod_material,d.codigo_material,d.descripcion_material,l.cod_proveedor,d.cod_linea_proveedor from subgrupos_material s join material_apoyo d on d.codigo_material=s.cod_material join proveedores_lineas l on l.cod_linea_proveedor=d.cod_linea_proveedor where s.cod_subgrupo=$codigo_registro and d.estado=1 order by 1)
   UNION (select d.codigo_material,0 as codigo_material,d.descripcion_material,l.cod_proveedor,d.cod_linea_proveedor from material_apoyo d join proveedores_lineas l on l.cod_linea_proveedor=d.cod_linea_proveedor where d.estado=1 and d.codigo_material not in (SELECT s.cod_material from subgrupos_material s join material_apoyo d on d.codigo_material=s.cod_material where s.cod_subgrupo=$codigo_registro and d.estado=1) $sqlCodigo $stringLineasX $sqlNombre $stringFormasX $stringAccionesX order by 1 $sqllimit)";
 // echo $sql;
