...
<br><center><table align='center' class='texto' width='70%' id='ventasSucursal'>
  <thead>
<tr><th style="background: #E6E1DF">Sucursal</th><th width="20%" style="background: #E6E1DF">Hora</th><th align="right" style="background: #E6E1DF">Cantidad Atenciones</th><th align="right" style="background: #E6E1DF">Monto</th><th align="right" style="background: #E6E1DF">% Cantidad</th><th align="right" style="background: #E6E1DF">% Monto</th>
</tr>
</thead>
<tbody>
<?php
...
while($datosSuc=mysqli_fetch_array($respSucursal)){ 
  ...
  $datosTotal=obtenerMontoVentasHoraTotal($fecha_iniconsulta,$fecha_finconsulta,$codigoSuc);
  ...
  for ($i=0; $i < 24 ; $i++) { 
    $hora=$i.":00 - ".$i.":59";   
    $datos=obtenerMontoVentasHora($fecha_iniconsulta,$fecha_finconsulta,$codigoSuc,$i);
    $cantHora=$datos[0];
    $montoHora=$datos[1];
    if($montoTotal>0){
    ...
    $arrPor[$codigoSuc.'_'.$i]=$porc;   
    $arrPorCant[$codigoSuc.'_'.$i]=$porcCnt;    
    ?><tr id="<?=$codigoSuc?>_<?=$i?>"><td><?=$nombreSuc?></td><td><b><?=$hora?></b></td><td align="right"><b><?=$datos[0]?></b></td><td align="right"><?=number_format($montoHora,2,'.',',')?></td><td id="c_<?=$codigoSuc?>_<?=$i?>" align="right"><?=number_format($porcCnt,2,'.','')." %"?></td><td align="right"><?=number_format($porc,2,'.','')." %"?></td></tr>
    <?php       
  }
