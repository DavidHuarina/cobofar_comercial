...
function recalculaCostos($codigoItem, $rpt_almacen, $enlaceCon){
	...
	while($datMateriales=mysqli_fetch_array($respMateriales)){
		...
		{	$fecha_consulta=$vector_final_fechas[$indice];
			...
			{	$nro_ingreso=$dat_ingresos[0];
				...
				if($codTipoIngreso==1002){       /*CUANDO ES POR AJUSTE O POR OTRO TIPO*/
			...
			{	$nro_salida=$dat_salidas[0];
				...
				if($codTipoSalida==1002){
					...
					if($nroFilasIngCambioItem==1){
						$codIngresoCambio=mysqli_result($respIngCambioItem,0,0);
...
function obtenerSaldoKardexAnterior($rpt_almacen, $rpt_item, $fecha, $enlaceCon){
